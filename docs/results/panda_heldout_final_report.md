...
The current implementation also includes an optional RoboCasa semantic bridge. This bridge can open a live RoboCasa `PickPlaceCoffee` episode in the WSL RoboCasa environment, extract the task object and fixture semantics, and project that semantic snapshot into the local MorphTAMP-X MuJoCo/Panda evaluation loop. This improves the grounding of the benchmark in RoboCasa-style manipulation tasks while keeping the evidence scope conservative: the reported results are still MuJoCo/Panda replay evidence, not full RoboCasa contact-rich policy execution.
...
## Ablation and Failure-Boundary Evidence

The companion ablation report checks whether the main methodological ingredients are actually supported by the current evidence rather than only appearing in the code. The reach-margin constraint is active: the selected design preserved a minimum held-out reach margin of 0.05115 m against the configured 0.05 m threshold. The singularity constraint is active: the selected design kept a minimum Panda Jacobian singular value of 0.24051 and a maximum condition number of 4.09616, compared with thresholds of 0.08 and 30. Multi-grasp selection is also active in the benchmark evidence, with grasp-candidate information recorded for 110 benchmark rows, a mean of 2.4 coarse candidates, and a mean of 2.0 full candidates.

The morphology benchmark also exposes a useful lower-cost failure boundary. The `short_arm` baseline had the lowest morphology cost among the preset baselines, 0.72208, but its success rate dropped to 0.84545 and its minimum reach margin was only 0.01611 m. In contrast, the selected optimized design had a slightly higher cost, 0.75712, but achieved full success under the held-out constraints. This supports the intended optimization story: the method is not merely minimizing length; it is finding the cheapest design that remains feasible under reachability and singularity constraints.
